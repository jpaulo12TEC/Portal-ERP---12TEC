...
// Sobre os Produtos ------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  interface Produto {
    ...
    const adicionarProduto = () => {
      if (!produto || !valorUnitario || !quantidade || !tipo || !categoriaSelecionada || !subcategoriaSelecionada) {
...
// Centros de Custo e OS's ------------------------------------------------------------------------------------------------------------------------------------------------------

// Adiciona centro de custo
  const adicionarCentroCusto = () => {
    setCentros(prevCentros => [...prevCentros, { centro: "", valor: "" }]);
  };
  const atualizarCentro = (index: number, campo: "centro" | "valor", valor: string) => {
    const novosCentros = [...centros];
    novosCentros[index][campo] = valor;
...
//Adicionar OS
  ...
  const adicionarOrdemServico = () => {
    setOrdensDeServico(prevOrdens => [...prevOrdens, { ordem: "", valor: "" }]);
  };

  // Função para atualizar o campo de uma ordem de serviço
  const atualizarOrdem= (index: number, campo: "ordem" | "valor", valor: string) => {
    const novasOrdens = [...ordensDeServico];
    novasOrdens[index][campo] = valor;
    setOrdensDeServico(novasOrdens);
  };

  // Função para remover uma ordem de serviço
  const removeOrdemServico = (index: number) => {
    setOrdensDeServico(ordensDeServico.filter((_, i) => i !== index));
  };
...
const somaCentros = centros.reduce((acc, centro) => acc + Number(centro.valor || 0), 0);
...
// ✅ Verifica se a soma dos centros está dentro da margem de erro de 1
const somaCentrosInvalida = Math.abs(somaCentros - valorFinal) > 1;
...
const somaOrdensInvalida = ordensDeServico.length > 0 && Math.abs(somaOrdens - valorFinal) > 1;
  ...
  if (somaCentrosInvalida) {
    mensagemErro += `❌ Soma dos centros de custo diferente de ${valorFinal}\n`;
  }
...
formaPagamentoGeral.forEach((pagamento, index) => {
  ...
  // Dados adicionais que precisam ser enviados para a outra tabela
    ...
    centros_de_custo: centros.map(item => `${item.centro};${calcularPercentual(item.valor)}`).join(';'),
...
if (centros.length > 0) {
  centros.forEach((centro) => {
    const percentual2 = (Number(centro.valor) / valorFinal) * 100;
    const centroData = {
      ...
      valor: centro.valor,      
      percentual: percentual2.toFixed(2),  // Percentual calculado com 2 casas decimais 
    };
...
const limparTodosOsForms = () => {
  ...
  return (
    <div className="mt-0 p-4 bg-white shadow-lg rounded-xl max-w-full">
      ...
      <div className={`content flex-1 p-1 min-h-screen w-90% `}
            style={{
              background: 'linear-gradient(to right, rgb(41, 9, 9), rgb(19, 0, 0))', // Gradiente linear
                  }}>
    ...
    <Card >
      <CardContent 
            className="border p-4 border-gray-300 h-auto"
            style={{
              background: 'linear-gradient(to right, rgb(240, 227, 227), rgb(255, 255, 255))', // Gradiente linear
            }}
            >
          ...
          <form className="space-y-0 mb-5 w-full">   
            ...
            <div className="p-0 flex flex-col w-1/4">
                  ...
                  <input
                    className="bg-yellow-100 px-3 py-2 border-1 border-gray-300 rounded-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 
    hover:border-gray-400 "
              ...
              <div className="flex flex-col w-1/4">
                  <label className="block text-sm font-large font-bold"> Data de lançamento</label>
                  <input
                    className="bg-yellow-100  px-3 py-2 border-1 border-gray-300 rounded-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 
    hover:border-gray-400 "
                    type="datetime-local"
...
<div className="flex flex-col w-3/4">
    ...
    />
           ...
           <div className="flex items-center ml-1 w-40 mt-5">
                  <label className="text-xm font-bold mr-5">Possui ICMS?</label>
                         <input
                           type="checkbox"
                           checked={temIcms}
                           onChange={(e) => setTemIcms(e.target.checked)}
                           className="mr-0"
                        />                       
                </div>
                {/* Se o ICMS for marcado, exibe o campo de cálculo */}
                {temIcms && (
                     <div className="flex flex-col w-1/4">
                     <label className="block text-sm font-large font-bold">ICMS (18%)</label>
                      <input
                           type="number"
                           value={(valorTotal * 0.18).toFixed(2)}
                          readOnly
                           className="bg-yellow-100 px-3 py-2 border-1 border-gray-300 rounded-md"
                             />
                      </div>
    ...
    <Card >
    <CardContent 
            className="border p-4 border-gray-300 h-auto  mt-[3px]"
            style={{
              background: 'linear-gradient(to right, rgb(240, 227, 227), rgb(255, 255, 255))', // Gradiente linear
            }}
            >
...
>
  <span className="group-hover:hidden">+ Parcela</span>
  <span className="hidden group-hover:inline">Adicionar Parcela</span>
</button>
              </div>
          </div>
        </form>
    ...
    <Card >
    <CardContent 
            className="border border-gray-300 h-auto p-0 mt-[3px]"
            style={{
              background: 'linear-gradient(to right, rgb(240, 227, 227), rgb(255, 255, 255))', // Gradiente linear
            }}
            >
           ...
           {/* Informações Gerais da Compra */}
          <form className="space-y-4 mb-0 w-full"> 

           {/* Layout com flexbox para dividir os cartões */}
           <div className="flex justify-between w-full space-x-4">

{/* Centro de Custo */}
<div className="w-1/2 space-y-0 mr-0">
  <div className="flex justify-between w-full items-center">
    
  <button
    type="button"
    onClick={adicionarCentroCusto}
    className="px-6 py-3 flex-1 text-gray-900 font-bold  transition duration-50 bg-gradient-to-r from-gray-100 to-gray-300 hover:from-gray-500 hover:to-gray-300 shadow-md hover:shadow-lg hover:text-white transform hover:scale-99"
  >
    + Centro de Custo
  </button>
  </div>
  ...
  {centros.map((item, index) => (
    <div key={index} className="flex flex-wrap  space-x-1 mb-0 bg-gradient-to-r border-r-1 border-gray-500 from-gray-100 to-gray-200 p-4">
      <div className="flex flex-col w-2/7">
        <label className="block text-sm font-large font-bold">Centro de Custo</label>
        <select
          value={item.centro}
          onChange={(e) => atualizarCentro(index, "centro", e.target.value)}
          className={`
            px-3 py-2 
            border-1 border-gray-300 h-11 rounded-md 
            transition-all duration-100 
            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 
            hover:border-gray-400 
            ${item.centro? "bg-green-100 border-green-700 border-1" : "bg-white"} 
          `}
        >
          <option value="">Selecione</option>
      ...
      <div className="flex flex-col w-2/7">
        <label className="block text-sm font-large font-bold">Valor</label>
        <input
          type="number"
          value={item.valor}
          onChange={(e) => atualizarCentro(index, "valor", e.target.value)}
          className={`
            px-3 py-2 
            border-1 border-gray-300 h-11 rounded-md 
            transition-all duration-100 
            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 
            hover:border-gray-400 
            ${item.valor? "bg-green-100 border-green-700 border-1" : "bg-white"} 
          `}
          placeholder="Valor"
        />
      </div>
      <div className="flex flex-col w-2/7">
        <label className="block text-sm font-large font-bold">Percentual</label>
        <input
          type="text"
          value={calcularPercentual(item.valor)}
          readOnly
          className="px-3 py-2 h-11 border rounded-md bg-yellow-100 w-auto text-sm font-bold"
        />
      </div>
      {/* Botão de remoção do centro de custo */}
      <button
        type="button"
        onClick={() => removeCentroCusto(index)}
        className="mx-auto block text-red-700 mt-4 inline-block font-bold"
      >
        ❌
      </button>
    </div>
...
<div className="w-1/2 space-y-0">
  ...
  {ordensDeServico.map((item, index) => (
    <div key={index} className="flex flex-wrap space-x-1 mb-0 bg-gradient-to-r border-l-1 border-gray-500 from-gray-200 to-gray-100 p-4">
      <div className="flex flex-col w-2/7">
        <label className="block text-sm font-large font-bold">Ordem de Serviço</label>
        <select
          value={item.ordem}
          onChange={(e) => atualizarOrdem(index, "ordem", e.target.value)}
          className={`
            px-3 py-2 
            border-1 border-gray-300 h-11 rounded-md 
            transition-all duration-100 
            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 
            hover:border-gray-400 
            ${item.ordem? "bg-green-100 border-green-700 border-1" : "bg-white"} 
          `}
        >
          <option value="">Selecione</option>
          <option value="OS-204565">OS-204565</option>
          <option value="OS-56859">OS-56859</option>
        </select>
      </div>
      <div className="flex flex-col w-2/7">
        <label className="block text-sm font-large font-bold">Valor</label>
        <input
          type="number"
          value={item.valor}
          onChange={(e) => atualizarOrdem(index, "valor", e.target.value)}
          className={`
            px-3 py-2 
            border-1 border-gray-300 h-11 rounded-md 
            transition-all duration-100 
      ...
      {/* Botão de remoção da ordem de serviço */}
      <button
        type="button"
    ...
    <Card>
      <CardContent
          className="border border-gray-300 p-5 mt-[3px]"
          style={{ background: "linear-gradient(to right, rgb(240, 227, 227), rgb(255, 255, 255))" }}
        >
          <h1 className="text-lg font-semibold mb-4">Cadastrar Produto/Serviço</h1>
...
<div className="flex flex-col w-1/4 ">
  ...
  />
          ...
          <button
            ...
            className="group mt-4 mb-20 w-50 hover:w-90  bg-gray-900 text-gray-200 font-bold px-5 py-2 rounded-sm shadow-sm border border-white transition-all duration-300 transform hover:scale-105 hover:bg-gradient-to-r hover:from-gray-800 hover:to-gray-900"
          >
            <span className="group-hover:hidden">+ Produto</span>
            <span className="hidden group-hover:inline">Adicionar Produto</span>
          </button>
