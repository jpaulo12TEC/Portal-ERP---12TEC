...
export default function PaginaInicialSSMA() {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<string>('ssma');
  const [menuActive, setMenuActive] = useState(false);

  const botoesPrincipais = [
    { label: 'TREINAMENTOS', icon: FileText, path: '/dashboard/ssma/treinamentos' },
    { label: 'DOCUMENTAÇÃO COLABORADORES', icon: FolderKanban, path: '' },
    { label: 'VENCIMENTOS ASO', icon: ClipboardList, path: '' },
    { label: 'VENCIMENTOS NR\'S', icon: ListChecks, path: '' },
    { label: 'VENCIMENTOS TODOS', icon: ListChecks, path: '' },
    { label: 'GERADOR DE CERTIFICADO', icon: FilePlus, path: '/dashboard/SSMA/criacaodecertificados' },
  ];
      ...
      <div className="flex items-center justify-between bg-[#200101] text-white shadow-md">
        <div className="flex space-x-4 w-full h-[40px] items-center">
          ...
          <div className="px-3 py-3 h-[50px]">
            <span className="w-full text-left">Menu SSMA</span>
          </div>
        </div>
