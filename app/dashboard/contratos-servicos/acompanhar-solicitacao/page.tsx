...
// Constante com os números válidos de 1 a 6
const ORCAMENTOS_VALIDOS = ['1','2','3','4','5','6'];

// Pergunta qual o orçamento vencedor
let orcamentoVencedor: string | null = null;
while (orcamentoVencedor === null) {
  const input = prompt(
    `Qual o orçamento vencedor? (Digite um número de 1 a 6)\nOpções válidas: ${ORCAMENTOS_VALIDOS.join(', ')}`
  );
  ...
  if (ORCAMENTOS_VALIDOS.includes(input.trim())) {
    orcamentoVencedor = input.trim();
  } else {
    alert('Opção inválida! Por favor digite um número de 1 a 6.');
  }
}
...
while (!fornecedor) {
  fornecedor = prompt('Informe o nome do fornecedor selecionado:');
  if (fornecedor === null) {
    setLoadingActionId(null);
    return;
  }
  if (!fornecedor.trim()) {
    alert('Campo obrigatório. Informe o nome do fornecedor.');
    fornecedor = null;
  }
}
...
while (!motivo) {
  motivo = prompt('Informe o motivo para selecionar este fornecedor:');
  if (motivo === null) {
    setLoadingActionId(null);
    return;
  }
  if (!motivo.trim()) {
    alert('Campo obrigatório. Informe o motivo.');
    motivo = null;
  }
}

// Agora você tem:
// orcamentoVencedor -> número de 1 a 6
// fornecedor -> nome do fornecedor escolhido
  ...
  try {
    const { error } = await supabase
      ...
      .update({
        ...
        orcamento_escolhido: orcamentoVencedor
      })
      .eq('id', id);
