...
setLoading(true); // Inicia o loading
  ...
  return (
    <div className={`flex flex-col h-screen ${menuActive ? 'ml-[300px]' : 'ml-[80px]'}`}>
      {/* Topbar */}
      <div className="flex items-center justify-between bg-[#1a0000] px-6 py-3 text-white shadow-md">
        <div className="flex items-center gap-4">
          <button
            onClick={() => window.history.back()}
            className="flex items-center gap-2 px-5 py-2 bg-[#4b0b0b] hover:bg-[#6a1414] text-white rounded-full transition-all shadow-sm"
          >
            <ArrowLeft size={18} />
            <span className="text-sm font-medium">Voltar</span>
          </button>
          <span className="text-base font-semibold tracking-wide">Cadastro de Fornecedor</span>
        </div>
